...
select date_format(now(), '%Y년 %m월 %d월 %h시 %i분 %s초') from dual;
...
select date_format('2023-08-30', '%Y년 %m월 %d일'),
...
select cast(1-2 as unsigned),
 ...
 -- 실수: float, double
 -- 시간: date, datetime
 -- LOB: CLOB(Charater Large OBject, BLOB(Binary Large OBject)